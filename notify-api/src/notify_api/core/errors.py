...
async def http_error_handler(request: Request, exc: HTTPException) -> JSONResponse:  # pylint: disable=unused-argument
    """Handler for error to transform default pydantic error object to gothinkster format."""
    return JSONResponse({'errors': [exc.detail]}, status_code=exc.status_code)


async def http_422_error_handler(request: Request, exc: HTTPException) -> JSONResponse:  # pylint: disable=unused-argument
