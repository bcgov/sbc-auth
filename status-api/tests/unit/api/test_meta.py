...
def test_meta_no_commit_hash(client):
    ...
    from status_api.version import __version__  # ylint: disable = import-outside - toplevel

    rv = client.get('/api/v1/meta/info')

    assert rv.status_code == 200
    assert rv.json == {'API': f'status_api/{__version__}'}
...
def test_meta_with_commit_hash(monkeypatch, client):
    ...
    from status_api.version import __version__  # pylint: disable = import-outside - toplevel
    ...
    rv = client.get('/api/v1/meta/info')
    assert rv.status_code == 200
    assert rv.json == {'API': f'status_api/{__version__}-{commit_hash}'}
