...
def upgrade():
    ...
    # we convert tasks.remarks from character type to array form ( check if there are arrays already, edge case )
    # we convert the column remarks type from character to ARRAY

    conn = op.get_bind()
    task_res = conn.execute("SELECT * FROM tasks;")
    task_list: List[Task] = task_res.fetchall()

    for task in task_list:
        if task.remarks is None:
            continue
        # A check if there are task remarks that is already wrapped in '{}', ideally, we would not hit this scenario
        if task.remarks.startswith('{') and task.remarks.endswith('}'):
            continue
        else:
            task_id = task.id
