...
def upgrade():
    ...
    op.bulk_insert(
        membership_status_table, [{"id": 5, "name": "PENDING_STAFF_REVIEW", "description": "Pending Staff Review"}]
    )

    # Find approved BCeID affidavit users.
    conn = op.get_bind()
    affidavits: List[Affidavit] = conn.execute(
        text("select * from affidavits where status_code='APPROVED' ")
    ).fetchall()
