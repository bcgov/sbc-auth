...
def test_entity(session):
    ...
    entity = EntityModel(
        business_identifier="CP1234567", business_number="791861073BC0001", name="Foobar, Inc.", corp_type_code="CP"
    )
    session.add(entity)
    session.commit()
...
def test_bootstrap_entity_with_sub_type(session):
    """Assert that an Entity can be retrieved via business identifier."""
    entity = EntityModel(business_identifier="tmp000123", name="Test", corp_type_code="TMP", corp_sub_type_code="ULC")
    session.add(entity)
    session.commit()
