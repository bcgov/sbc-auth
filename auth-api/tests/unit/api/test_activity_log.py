...
from tests.utilities.factory_utils import (factory_auth_header,
                                           factory_user_model, factory_activity_log_model)


def test_fetch_log_no_content(client, jwt, session):  # pylint:disable=unused-argument
    """Assert that the none can be fetched."""
    headers = factory_auth_header(jwt=jwt, claims=TestJwtClaims.staff_role)
    rv = client.get('/api/v1/activity-logs', headers=headers, content_type='application/json')
...
def test_fetch_activity_log(client, jwt, session):  # pylint:disable=unused-argument
    ...
    headers = factory_auth_header(jwt=jwt, claims=TestJwtClaims.staff_role)
    rv = client.get('/api/v1/activity-logs',
    ...
    rv = client.get('/api/v1/activity-logs?itemType=ORG',
                    headers=headers, content_type='application/json')
