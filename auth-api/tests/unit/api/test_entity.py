...
def test_temp_business_with_subtype(client, jwt, session, test_name, legal_type):  # pylint:disable=unused-argument
    ...
    headers = factory_auth_header(jwt=jwt, claims=TestJwtClaims.system_role)
    rv = client.post(
        "/api/v1/entities", data=json.dumps(temp_business_json), headers=headers, content_type="application/json"
    )
    assert rv.status_code == HTTPStatus.CREATED
    assert schema_utils.validate(rv.json, "business")[0]

    entity_rv = client.get(
        "/api/v1/entities/{}".format(temp_business_json["businessIdentifier"]),
        headers=headers,
        content_type="application/json",
    )

    assert entity_rv.status_code == HTTPStatus.OK
...
def test_add_entity_invalid_returns_400(client, jwt, session):  # pylint:disable=unused-argument
    ...
    rv = client.post(
        "/api/v1/entities", data=json.dumps(TestEntityInfo.invalid), headers=headers, content_type="application/json"
    )
...
def test_add_entity_no_auth_returns_401(client, session):  # pylint:disable=unused-argument
    ...
    rv = client.post(
        "/api/v1/entities", data=json.dumps(TestEntityInfo.entity1), headers=None, content_type="application/json"
    )
...
def test_add_contact(client, jwt, session):  # pylint:disable=unused-argument
    ...
    headers = factory_auth_header(jwt=jwt, claims=TestJwtClaims.system_role)
    rv = client.post(
        "/api/v1/entities/{}/contacts".format(TestEntityInfo.entity1["businessIdentifier"]),
        headers=headers,
...
def test_update_entity_success(client, jwt, session):  # pylint:disable=unused-argument
    ...
    rv = client.patch(
        "/api/v1/entities/{}".format(TestEntityInfo.entity2["businessIdentifier"]),
...
def test_update_entity_failures(client, jwt, session):  # pylint:disable=unused-argument
    ...
    rv = client.patch(
        "/api/v1/entities/{}".format("1234"), data=json.dumps(TestEntityInfo.entity2), content_type="application/json"
    )
    assert rv.status_code == HTTPStatus.UNAUTHORIZED
...
def test_authorizations_for_staff_returns_200(client, jwt, session):  # pylint:disable=unused-argument
    ...
    rv = client.get(f"/api/v1/entities/{inc_number}/authorizations", headers=headers, content_type="application/json")
...
def test_delete_entity_unauthorized(client, jwt, session):  # pylint:disable=unused-argument
    ...
    rv = client.delete(
        "/api/v1/entities/{}".format(TestEntityInfo.entity1.get("businessIdentifier")),
        headers=headers,
        content_type="application/json",
    )
    assert rv.status_code == HTTPStatus.UNAUTHORIZED
...
def test_reset_passcode_success(client, jwt, session):  # pylint:disable=unused-argument
    ...
    client.post(
        "/api/v1/entities/{}/contacts".format(TestEntityInfo.entity1.get("businessIdentifier")),
        headers=headers,
        data=json.dumps(TestContactInfo.contact1),
        content_type="application/json",
    )
