...
def test_update_user_terms_of_use_no_jwt(client, jwt, session):  # pylint:disable=unused-argument
    ...
    # post token with updated claims
    input_data = json.dumps({"invalid": True})
    rv = client.patch("/api/v1/users/@me", data=input_data, content_type="application/json")
    assert rv.status_code == HTTPStatus.UNAUTHORIZED
...
def test_staff_get_user(client, jwt, session):  # pylint:disable=unused-argument
    ...
    rv = client.get(
        "/api/v1/users/{}".format(TestJwtClaims.public_user_role["preferred_username"]),
        headers=headers,
...
def test_staff_search_users(client, jwt, session):  # pylint:disable=unused-argument
    ...
    rv = client.get(
        "/api/v1/users?lastname={}".format(TestJwtClaims.no_role["lastname"]),
        headers=headers,
        content_type="application/json",
    )
    assert rv.status_code == HTTPStatus.OK
...
def test_get_orgs_for_user(client, jwt, session, keycloak_mock):  # pylint:disable=unused-argument
    ...
    rv = client.post(
        "/api/v1/orgs", headers=headers, data=json.dumps(TestOrgInfo.org1), content_type="application/json"
    )
...
def test_add_bceid_user(client, jwt, session):  # pylint:disable=unused-argument
    ...
    rv = client.post(
        ...
        headers=headers,
        content_type="application/json",
        data=json.dumps({"firstName": "John", "lastName": "Doe"}),
    )
    assert rv.status_code == HTTPStatus.CREATED
    ...
    rv = client.post(
        ...
        headers=headers,
        content_type="application/json",
        data=json.dumps({"firstName": "John-New", "lastName": "Doe"}),
    )
...
def test_user_post_during_login(client, jwt, session):  # pylint:disable=unused-argument
    ...
    time.sleep(1)
    rv = client.post(
        "/api/v1/users", headers=headers, data=json.dumps({"isLogin": True}), content_type="application/json"
    )
    assert schema_utils.validate(rv.json, "user_response")[0]
    assert login_time != rv.json.get("loginTime")
...
def test_get_rejected_affidavit(client, jwt, session, keycloak_mock, gcs_mock):  # pylint:disable=unused-argument
    ...
    rv = client.get(
        f"/api/v1/users/{TestJwtClaims.public_user_role.get('sub')}/affidavits?status=REJECTED",
        headers=headers,
        content_type="application/json",
    )
    assert rv.status_code == HTTPStatus.OK
