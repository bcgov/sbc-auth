...
def test_preflight_account(app, client, jwt, session):
    ...
    rv = client.options(
        "/api/v1/accounts/1/products/1/authorizations", headers={"Access-Control-Request-Method": "GET"}
    )
...
def test_preflight_affiliation_invitation(app, client, jwt, session):
    ...
    rv = client.options(
        "/api/v1/affiliationInvitations/1/authorization/ACTION", headers={"Access-Control-Request-Method": "PATCH"}
    )
    assert rv.status_code == HTTPStatus.OK
...
def test_preflight_entity(app, client, jwt, session):
    ...
    rv = client.options(
        "/api/v1/entities/BUSINESS_IDENTIFIER/contacts", headers={"Access-Control-Request-Method": "GET"}
    )
    assert rv.status_code == HTTPStatus.OK
    assert_access_control_headers(rv, "*", "DELETE, GET, POST, PUT")

    rv = client.options(
        "/api/v1/entities/BUSINESS_IDENTIFIER/authorizations", headers={"Access-Control-Request-Method": "GET"}
    )
    assert rv.status_code == HTTPStatus.OK
    assert_access_control_headers(rv, "*", "GET")
...
def test_preflight_org(app, client, jwt, session):
    ...
    rv = client.options(
        "/api/v1/orgs/affiliation/BUSINESS_IDENTIFIER", headers={"Access-Control-Request-Method": "GET"}
    )
    assert rv.status_code == HTTPStatus.OK
    assert_access_control_headers(rv, "*", "GET")

    rv = client.options(
        "/api/v1/orgs/1/affiliations/BUSINESS_IDENTIFIER", headers={"Access-Control-Request-Method": "GET"}
    )
    assert rv.status_code == HTTPStatus.OK
...
def test_preflight_org_permissions(app, client, jwt, session):
    ...
    rv = client.options(
        "/api/v1/permissions/ORG_STATUS/MEMBERSHIP_TYPE", headers={"Access-Control-Request-Method": "GET"}
    )
    assert rv.status_code == HTTPStatus.OK
    assert_access_control_headers(rv, "*", "GET")
