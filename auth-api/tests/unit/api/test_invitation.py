...
def test_add_invitation(client, jwt, session, keycloak_mock, org_info, role, claims):  # pylint:disable=unused-argument
    ...
    rv = client.post(
        "/api/v1/invitations",
        data=json.dumps(factory_invitation(org_id=org_id, membership_type=role)),
        headers=headers,
...
@pytest.mark.parametrize(
    "org_info, role, claims, source, exp_status",
    [
        (
        ...
        (
            TestOrgInfo.org_regular_bceid,
            "ADMIN",
            TestJwtClaims.public_bceid_account_holder_user,
...
def test_accept_public_users_invitation(
    ...
    rv = client.get(
        f"/api/v1/orgs/{org_id}/members?status={exp_status.name}", headers=headers, content_type="application/json"
    )
    dictionary = json.loads(rv.data)
