...
def test_simple_org_search(client, jwt, session, keycloak_mock):  # pylint:disable=unused-argument
    ...
    org_inactive = OrgModel(
        name="INACTIVE TST ORG",
        branch_name="INACTIVE TST BRANCH NAME",
        type_code=OrgType.PREMIUM.value,
        status_code=OrgStatus.INACTIVE.value,
    ).save()

    org_branch_1 = OrgModel(
        name="TST ORG NAME 1",
        branch_name="TST BRANCH 1",
        type_code=OrgType.PREMIUM.value,
        status_code=OrgStatus.ACTIVE.value,
    ).save()

    org_branch_2 = OrgModel(
        name="TST ORG NAME 2",
    ...
    org_no_branch_1 = OrgModel(
        name="TST ORG NO BRANCH Name 1", type_code=OrgType.PREMIUM.value, status_code=OrgStatus.ACTIVE.value
    ).save()

    org_no_branch_2 = OrgModel(
        name="TST ORG NO BRANCH name 2", type_code=OrgType.PREMIUM.value, status_code=OrgStatus.ACTIVE.value
    ).save()

    headers = factory_auth_header(jwt=jwt, claims=TestJwtClaims.manage_eft_role)

    rv = client.get(
        f"/api/v1/orgs/simple?statuses={OrgStatus.INACTIVE.value}", headers=headers, content_type="application/json"
    )
    ...
    rv = client.get(
        f"/api/v1/orgs/simple?statuses={OrgStatus.ACTIVE.value}&excludeStatuses=true",
        headers=headers,
    ...
    rv = client.get(
        f"/api/v1/orgs/simple?searchText={org_no_branch_2.id}", headers=headers, content_type="application/json"
    )
