...
def test_fetch_tasks_with_params(test_name, client, jwt, endpoint, session):  # pylint:disable=unused-argument
    ...
    headers = factory_auth_header(jwt=jwt, claims=TestJwtClaims.staff_role)
    rv = client.get(f"/api/v1/tasks?{endpoint}", headers=headers, content_type="application/json")
    item_list = rv.json
...
def test_fetch_tasks_end_of_day(client, jwt, session):
    ...
    headers = factory_auth_header(jwt=jwt, claims=TestJwtClaims.staff_role)
    rv = client.get(
        "/api/v1/tasks?startDate=2022-7-10&endDate=2022-7-10", headers=headers, content_type="application/json"
    )
    item_list = rv.json
...
def test_put_task_product(client, jwt, session, keycloak_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    update_task_payload = {"relationshipStatus": ProductSubscriptionStatus.ACTIVE.value}
...
@mock.patch("auth_api.services.affiliation_invitation.RestService.get_service_account_token", mock_token)
def test_tasks_on_account_creation(
    client,
    jwt,
    session,
