...
def test_get_user_authorizations_for_entity(session, monkeypatch):  # pylint:disable=unused-argument
    ...
    patch_token_info({"sub": str(user.keycloak_guid), "realm_access": {"roles": ["basic"]}}, monkeypatch)
    ...
    patch_token_info({"sub": str(uuid.uuid4()), "realm_access": {"roles": ["basic"]}}, monkeypatch)
    authorization = Authorization.get_user_authorizations_for_entity(entity.business_identifier)
    assert authorization is not None
    ...
    patch_token_info(
        {"loginSource": "PASSCODE", "username": "INVALID", "realm_access": {"roles": ["basic"]}}, monkeypatch
    )
    authorization = Authorization.get_user_authorizations_for_entity(entity.business_identifier)
    ...
    patch_token_info({"loginSource": "", "realm_access": {"roles": ["staff"]}}, monkeypatch)
    authorization = Authorization.get_user_authorizations_for_entity(entity.business_identifier)
...
def test_get_user_authorizations_for_entity_service_account(session, monkeypatch):
    ...
    patch_token_info(
        {"loginSource": "", "realm_access": {"roles": ["system"]}, "product_code": "INVALIDCP"}, monkeypatch
    )
    authorization = Authorization.get_user_authorizations_for_entity(entity.business_identifier)
...
def test_check_auth(session, monkeypatch):  # pylint:disable=unused-argument
    ...
    patch_token_info(
        {"realm_access": {"roles": ["staff", "create_accounts"]}, "sub": str(user.keycloak_guid)}, monkeypatch
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Test OK when member of the org.", does_not_raise(), {}, True, False, ProductCode.BUSINESS.value),
        (
            "Test OK when business identifier provided, not affiliated...",
            does_not_raise(),
            {"business_identifier": "SOME_NOT_AFFILIATED"},
            False,
            False,
            ProductCode.BUSINESS.value,
        ),
        ...
        (
            ...
            does_not_raise(),
            {"org_id": 123, "business_identifier": "SOME_NOT_AFFILIATED"},
            False,
            False,
            ProductCode.BUSINESS.value,
        ),
...
def test_check_auth_system_path(
    session,
    monkeypatch,
    test_desc,
    test_expect,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Test 403 when org member, but no role checks provided in kwargs.", pytest.raises(Forbidden), {}, True, False),
        (
...
def test_check_auth_public_user_path(
    session, monkeypatch, test_desc, test_expect, additional_kwargs, is_org_member, is_entity_affiliated
):
...
def test_get_user_authorizations_for_entity_with_multiple_affiliations(
    ...
    patch_token_info({"sub": str(user_2.keycloak_guid), "realm_access": {"roles": ["basic"]}}, monkeypatch)
    authorization = Authorization.get_user_authorizations_for_entity(entity.business_identifier)
    assert authorization is not None
    assert authorization.get("orgMembership", None) == membership.membership_type_code
