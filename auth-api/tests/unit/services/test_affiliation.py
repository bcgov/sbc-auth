...
def test_create_affiliation(session, auth_mock, monkeypatch, environment):  # pylint:disable=unused-argument
    ...
    affiliation = AffiliationService.create_affiliation(
        org_id, business_identifier, environment, TestEntityInfo.entity_lear_mock["passCode"]
    )
    assert affiliation
...
@pytest.mark.parametrize("environment", ["test", None])
def test_create_affiliation_with_passcode_no_passcode_input(
    session, auth_mock, environment
...
def test_create_affiliation_firms(session, auth_mock, monkeypatch, environment):  # pylint:disable=unused-argument
    ...
    affiliation = AffiliationService.create_affiliation(
        org_id, business_identifier, environment, TestEntityInfo.entity_lear_mock3["passCode"]
    )
    assert affiliation
    assert affiliation.entity.identifier == entity_service.identifier
    assert affiliation.as_dict()["organization"]["id"] == org_dictionary["id"]
...
@pytest.mark.parametrize(
    "test_name, org_type, should_succeed, environment",
    [
...
@mock.patch("auth_api.services.affiliation_invitation.RestService.get_service_account_token", mock_token)
@pytest.mark.parametrize("environment", ["test", None])
def test_create_affiliation_firms_party_with_additional_space(session, auth_mock, monkeypatch, environment):
    """Assert that an Affiliation can be created."""
    patch_get_firms_parties(monkeypatch)
    entity_service = factory_entity_service(entity_info=TestEntityInfo.entity_lear_mock3)
    entity_dictionary = entity_service.as_dict()
    ...
    affiliation = AffiliationService.create_affiliation(org_id, business_identifier, environment, pass_code)
    assert affiliation
    assert affiliation.entity.identifier == entity_service.identifier
    assert affiliation.as_dict()["organization"]["id"] == org_dictionary["id"]
...
@mock.patch("auth_api.services.affiliation_invitation.RestService.get_service_account_token", mock_token)
@pytest.mark.parametrize("environment", ["test", None])
def test_create_affiliation_firms_party_not_valid(session, auth_mock, monkeypatch, environment):
    """Assert that an Affiliation can be created."""
    patch_get_firms_parties(monkeypatch)
    entity_service = factory_entity_service(entity_info=TestEntityInfo.entity_lear_mock3)
    entity_dictionary = entity_service.as_dict()
...
def test_find_affiliated_entities_by_org_id(session, auth_mock, environment):  # pylint:disable=unused-argument
    ...
    AffiliationService.create_affiliation(
        org_id, business_identifier1, environment, TestEntityInfo.entity_lear_mock["passCode"]
    )
...
@pytest.mark.parametrize("environment", ["test", None])
def test_find_affiliated_entities_by_org_id_no_affiliation(
    session, auth_mock, environment
):  # pylint:disable=unused-argument
...
@pytest.mark.parametrize("environment", ["test", None])
def test_delete_affiliation_no_affiliation(
    session, auth_mock, monkeypatch, environment
):  # pylint:disable=unused-argument
...
@pytest.mark.parametrize("environment", ["test", None])
def test_delete_affiliation_reset_passcode(
    session, auth_mock, monkeypatch, environment
):  # pylint:disable=unused-argument
    """Assert that an affiliation can be deleted."""
    entity_service = factory_entity_service(TestEntityInfo.entity_lear_mock)
...
def test_fix_stale_affiliations(session, auth_mock, nr_mock, system_user_mock, environment):
    ...
    affiliation1 = AffiliationService.create_affiliation(
        org_id, business_identifier=nr["business_identifier"], environment=environment
    )
    ...
    affiliation2 = AffiliationService.create_affiliation(
        org_id, business_identifier=business["business_identifier"], environment=environment
    )
...
def test_create_affiliation_multiple_environments(session, auth_mock, monkeypatch):  # pylint:disable=unused-argument
    ...
    affiliation_dev = AffiliationService.create_affiliation(
        org_id, business_identifier, "dev", TestEntityInfo.entity_lear_mock["passCode"]
    )
    ...
    affiliation_test = AffiliationService.create_affiliation(
        org_id, business_identifier, "test", TestEntityInfo.entity_lear_mock["passCode"]
    )
...
@pytest.mark.parametrize("environment", ["test", None])
def test_find_affiliation_multiple_environments(
    session, auth_mock, monkeypatch, environment
):  # pylint:disable=unused-argument
...
def test_delete_affiliation_multiple_environments(
    session, auth_mock, monkeypatch, environment
):  # pylint:disable=unused-argument
