...
from auth_api.utils.constants import GROUP_ACCOUNT_HOLDERS, GROUP_STAFF
...
from tests.utilities.factory_utils import convert_org_to_staff_org, factory_membership_model, factory_product_model, factory_user_model
...
@mock.patch("auth_api.services.affiliation_invitation.RestService.get_service_account_token", mock_token)
@pytest.mark.parametrize("group_check", [GROUP_ACCOUNT_HOLDERS, GROUP_STAFF])
def test_remove_member_removes_group_to_the_user(session, monkeypatch, group_check):  # pylint:disable=unused-argument
    ...
    user2 = factory_user_model(TestUserInfo.get_user_with_kc_guid(kc_guid=kc_user2.id))
    if group_check == GROUP_STAFF:
        # Group is added manually by ops.
        keycloak_service.add_user_to_group(kc_user2.id, GROUP_STAFF)

    # Add a membership to the user for the org created
    member_type = 'ADMIN' if group_check == GROUP_STAFF else 'COORDINATOR'
