...
from auth_api.services.authorization import Authorization as AuthorizationService, is_competent_authority
...
def post_entity():
    ...
    entity = EntityService.find_by_business_identifier(
        request_json.get("businessIdentifier"), allowed_roles=ALL_ALLOWED_ROLES
    )
    if entity:
        return entity.as_dict(), HTTPStatus.ACCEPTED
...
def patch_entity(business_identifier):
    ...
    try:
        if passcode_reset:
            entity = EntityService.reset_passcode(
                business_identifier, email_addresses=request_json.get("passcodeResetEmail", None)
            )
        else:
...
def get_entity_contact(business_identifier):
    ...
    if (entity := EntityService.find_by_business_identifier(business_identifier, skip_auth=True)) and (
        ...
        return ContactService(contact).as_dict(masked_email_only=True), HTTPStatus.OK
    return jsonify({"message": f"Contacts for {business_identifier} was not found."}), HTTPStatus.NOT_FOUND
...
def post_entity_contact(business_identifier):
    ...
    try:
        entity = EntityService.find_by_business_identifier(business_identifier, allowed_roles=ALL_ALLOWED_ROLES)
        if entity:
            response, status = entity.add_contact(request_json).as_dict(), HTTPStatus.CREATED
        else:
            response, status = {"message": "The requested business could not be found."}, HTTPStatus.NOT_FOUND
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
    return response, status
...
def put_entity_contact(business_identifier):
    ...
    try:
        entity = EntityService.find_by_business_identifier(business_identifier, allowed_roles=ALL_ALLOWED_ROLES)
        if entity:
            response, status = entity.update_contact(request_json).as_dict(), HTTPStatus.OK
        else:
            response, status = {"message": "The requested business could not be found."}, HTTPStatus.NOT_FOUND
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
...
def delete_entity_contact(business_identifier):
    ...
    try:
        ...
        if entity:
            response, status = entity.delete_contact().as_dict(), HTTPStatus.OK
        else:
            response, status = {"message": "The requested business could not be found."}, HTTPStatus.NOT_FOUND
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
