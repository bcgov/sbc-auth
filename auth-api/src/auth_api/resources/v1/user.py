...
def post_user():
    ...
    try:
        ...
        if Role.STAFF.name in roles and Role.GOV_ACCOUNT_USER.value in roles:
            current_app.logger.info("Removing GOV_ACCOUNT_USER role from STAFF user")
...
def get_current_user():
    ...
    try:
        response, status = UserService.find_by_jwt_token().as_dict(), HTTPStatus.OK
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
    return response, status
...
def patch_current_user():
    ...
    try:
        response, status = (
            UserService.update_terms_of_use(is_terms_accepted, version).as_dict(),
            HTTPStatus.OK,
        )
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
...
def get_user_contacts():
    ...
    token = g.jwt_oidc_token_info
    if not token:
        return {"message": "Authorization required."}, HTTPStatus.UNAUTHORIZED

    try:
        response, status = UserService.get_contacts(), HTTPStatus.OK
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
    return response, status
...
def post_user_contact():
    ...
    try:
        response, status = UserService.add_contact(request_json).as_dict(), HTTPStatus.CREATED
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
    return response, status
...
def put_user_contact():
    ...
    if not valid_format:
        return {"message": schema_utils.serialize(errors)}, HTTPStatus.BAD_REQUEST
    try:
        response, status = UserService.update_contact(request_json).as_dict(), HTTPStatus.OK
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
    return response, status
...
def delete_user_contact():
    ...
    try:
        response, status = UserService.delete_contact().as_dict(), HTTPStatus.OK
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
    return response, status
...
def get_user_organizations():
    ...
    try:
        ...
        if not user:
            response, status = {"message": "User not found."}, HTTPStatus.NOT_FOUND
        else:
            all_orgs = OrgService.get_orgs(user.identifier)
            orgs = OrgSchema().dump(all_orgs, many=True)
            response, status = jsonify({"orgs": orgs}), HTTPStatus.OK
...
def get_user_org_membership(org_id):
    ...
    try:
        ...
        if not user:
            ...
            membership = MembershipService.get_membership_for_org_and_user_all_status(
                org_id=org_id, user_id=user.identifier
            )
            response, status = MembershipSchema(exclude=["org"]).dump(membership), HTTPStatus.OK
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
    return response, status
...
def post_user_affidavit(user_guid):
    ...
    try:
        response, status = AffidavitService.create_affidavit(request_json).as_dict(), HTTPStatus.OK
    except BusinessException as exception:
        response, status = {"code": exception.code, "message": exception.message}, exception.status_code
    return response, status
...
def get_user_authorizations():
    ...
    return AuthorizationService.get_user_authorizations(sub), HTTPStatus.OK
