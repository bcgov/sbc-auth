...
def get_affiliation_invitations():
    ...
    try:
        ...
        if org_id:
            data = AffiliationInvitationService.get_all_invitations_with_details_related_to_org(
                org_id=org_id, search_filter=search_filter
            )
        else:
            data = AffiliationInvitationService.search_invitations(search_filter=search_filter)
        ...
        response, status = {"affiliationInvitations": data}, HTTPStatus.OK
...
def patch_affiliation_invitation(affiliation_invitation_id):
    ...
    try:
        ...
        if affiliation_invitation is None:
            ...
            user = UserService.find_by_jwt_token()
            response, status = (
                affiliation_invitation.update_affiliation_invitation(user, origin, request_json).as_dict(
                    mask_email=True
                ),
                HTTPStatus.OK,
            )
...
def accept_affiliation_invitation_token(affiliation_invitation_id, affiliation_invitation_token):
    ...
    try:
        if not (user := UserService.find_by_jwt_token()):
            ...
            affiliation_invitation_id = (
                AffiliationInvitationService.validate_token(
                    affiliation_invitation_token, int(affiliation_invitation_id)
                )
                .as_dict()
                .get("id")
            )
...
def patch_affiliation_invitation_authorization(affiliation_invitation_id, authorize_action):
    ...
    try:
        ...
        if authorize_action == "accept":
            ...
            response, status = (
                AffiliationInvitationService.refuse_affiliation_invitation(
                    invitation_id=affiliation_invitation_id, user=user
                ).as_dict(mask_email=True),
                HTTPStatus.OK,
            )
        else:
            err = {"code": 400, "message": f"{authorize_action} is not supported on this endpoint"}
            raise BusinessException(err, HTTPStatus.BAD_REQUEST)
...
def _verify_permissions(user, affiliation_invitation_id):
    ...
    if not UserService.is_user_in_membership_roles(user=user, org_id=to_org_id):
        raise BusinessException(Error.NOT_AUTHORIZED_TO_PERFORM_THIS_ACTION, None)
