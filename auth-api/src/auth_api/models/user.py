...
class User(BaseModel):
    ...
    contacts = relationship(
        "ContactLink", primaryjoin="User.id == ContactLink.user_id", lazy="select", back_populates="user"
    )
    orgs = relationship(
        "Membership",
        primaryjoin="and_(User.id == Membership.user_id,  or_(Membership.status == "
        + str(Status.ACTIVE.value)
        + ", Membership.status == "
        + str(Status.PENDING_APPROVAL.value)
        + "))",
        lazy="select",
        back_populates="user",
    )  # noqa:E127
    ...
    def create_from_jwt_token(cls, first_name: str, last_name: str, **kwargs):
        ...
        if token:
            ...
            logger.debug(f"Creating user from JWT:{token}; User:{user}")
    ...
    def update_from_jwt_token(
        ...
        **kwargs,
        ...
        logger.debug(f"Updating user from JWT:{token}; User:{user}")
    ...
    def update_terms_of_use(cls, is_terms_accepted, terms_of_use_version, **kwargs):
        ...
        if user_from_context.token_info:
            ...
            logger.debug(
                f"Updating users Terms of use is_terms_accepted:{is_terms_accepted}; "
    ...
    def find_users_by_org_id_by_status_by_roles(cls, org_id: int, roles, status=Status.ACTIVE.value):
        ...
        return (
            ...
            .join(
                MembershipModel,
                (User.id == MembershipModel.user_id)
