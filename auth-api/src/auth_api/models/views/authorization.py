...
from auth_api.utils.util import safe_int
...
class Authorization(db.Model):
    ...
    @classmethod
    def find_user_authorization_by_business_number(
        cls, business_identifier: str, keycloak_guid: uuid = None, org_id: int = None, is_staff=None
    ):
        """Return authorization view object."""
        auth = None
        if keycloak_guid and business_identifier and org_id:
            auth = cls.query.filter_by(
                keycloak_guid=keycloak_guid, business_identifier=business_identifier, org_id=safe_int(org_id)
            ).one_or_none()
        elif business_identifier and org_id:
            auth = cls.query.filter_by(business_identifier=business_identifier, org_id=safe_int(org_id)).first()
    ...
    def find_user_authorization_by_org_id(cls, keycloak_guid: uuid, org_id: int):
        """Return authorization view object."""
        return cls.query.filter_by(keycloak_guid=keycloak_guid, org_id=safe_int(org_id)).one_or_none()
    ...
    def find_authorization_for_admin_by_org_id(cls, org_id: int):
        ...
        return cls.query.filter_by(org_id=safe_int(org_id), org_membership=ADMIN).first()

    @classmethod
    def find_account_authorization_by_org_id_and_product_for_user(cls, keycloak_guid: uuid, org_id: int, product: str):
        """Return authorization view object."""
        return cls.query.filter_by(
            keycloak_guid=keycloak_guid, org_id=safe_int(org_id), product_code=product
        ).one_or_none()

    @classmethod
    def find_account_authorization_by_org_id_and_product(cls, org_id: int, product: str):
        """Return authorization view object."""
        return cls.query.filter_by(org_id=safe_int(org_id), product_code=product, org_membership=ADMIN).first()
