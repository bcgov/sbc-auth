...
class RestService:
    ...
    def _invoke(rest_method, endpoint, token=None,  # pylint: disable=too-many-arguments
        ...
        try:
            ...
            current_app.logger.error(f"HTTPError on POST {endpoint} with status code {response.status_code if response else ''}")
    ...
    def get(endpoint, token=None,  # pylint: disable=too-many-arguments
        ...
        try:
            ...
            current_app.logger.error(f"HTTPError on GET {endpoint} with status code {response.status_code if response else ''}")
            if response and response.status_code >= 500:
                raise ServiceUnavailableException(exc) from exc
            raise exc
        finally:
