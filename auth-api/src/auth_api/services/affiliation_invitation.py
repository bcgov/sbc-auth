...
class AffiliationInvitation:
    ...
    def _validate_prerequisites(
        ...
        if (contact and not contact.email) and affiliation_invitation_type != AffiliationInvitationType.REQUEST:
            raise BusinessException(Error.INVALID_BUSINESS_EMAIL, None)
        ...
        if AffiliationInvitationModel.find_invitations_by_org_entity_ids(
            from_org_id=from_org_id, entity_id=entity.identifier
        ):
            raise BusinessException(Error.DATA_ALREADY_EXISTS, None)
    ...
    def _get_invitation_email(
        ...
        if invitation_type := AffiliationInvitationType.from_value(
            ...
            return AffiliationInvitation.get_invitation_email(
                affiliation_invitation_type=invitation_type, entity=entity, org_id=org_id
            )
        ...
        **kwargs,
        ...
        if from_org.access_type == AccessType.ANONYMOUS.value:  # anonymous account never get bceid or bcsc choices
            ...
            default_login_option_based_on_accesstype = (
                LoginSource.BCSC.value if from_org.access_type == AccessType.REGULAR.value else LoginSource.BCEID.value
            )
        ...
        affiliation_invitation_info["recipientEmail"] = AffiliationInvitation._get_invitation_email(
            affiliation_invitation_info=affiliation_invitation_info, entity=entity, org_id=to_org_id
        )

        affiliation_invitation = AffiliationInvitationModel.create_from_dict(
            affiliation_invitation_info, user.identifier
        )
        confirmation_token = AffiliationInvitation.generate_confirmation_token(
            affiliation_invitation.id, from_org_id, to_org_id, business_identifier
        )
    ...
    def update_affiliation_invitation(self, user, invitation_origin, affiliation_invitation_info: Dict):
        ...
        if not new_status or new_status == InvitationStatus.PENDING.value:
            # Resend invitation
            confirmation_token = AffiliationInvitation.generate_confirmation_token(
                self._model.id, self._model.from_org_id, self._model.to_org_id, self._model.entity.business_identifier
            )
    ...
    def search_invitations(search_filter: AffiliationInvitationSearch, mask_email=True):
        ...
        invitation_models = AffiliationInvitation._filter_request_invites_role_based(
            affiliation_invitation_models=searched_invitations, org_id=org_id
        )
    ...
    def send_affiliation_invitation_authorization_email(
        affiliation_invitation: AffiliationInvitationModel, is_authorized: bool
    ):
    ...
    def validate_token(token, affiliation_invitation_id: int):
        ...
        try:
            ...
            if affiliation_invitation_id != token_invitation_id:
                raise BusinessException(Error.INVALID_AFFILIATION_INVITATION_TOKEN, None)
    ...
    @staticmethod
    @user_context
    def accept_affiliation_invitation(
        affiliation_invitation_id,
        ...
        **kwargs,
        ...
        affiliation_invitation.invitation_status = InvitationStatusModel.get_status_by_code(
            InvitationStatus.ACCEPTED.value
        )
        affiliation_invitation.save()

        if affiliation_invitation.type == AffiliationInvitationType.REQUEST.value:
            AffiliationInvitation.send_affiliation_invitation_authorization_email(
    ...
    def refuse_affiliation_invitation(invitation_id: int, user: UserService):
        ...
        AffiliationInvitation.send_affiliation_invitation_authorization_email(
            affiliation_invitation=invitation, is_authorized=False
        )
