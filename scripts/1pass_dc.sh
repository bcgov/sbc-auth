...
usage() {
  cat <<-EOF
  A helper script to get the secrcts from 1password' vault and create new dc in openshift.
  Usage: ./1pass_dc.sh [-h -d <subdomainName> -u <accountName>]
                        -k <secretKey>
                        -p <masterPassword>
                        -e <environment>
                        -v <vaultDetails>
...
# Create deployment config in Openshift
oc process -f openshift/templates/dc.json \
     -p NAME=${APP_NAME} \
     -p TAG_NAME=${VERSION} \
 | oc apply -f -
