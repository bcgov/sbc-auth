...
def process(email_msg: dict) -> dict:
    """Build the email for Account Suspended notification."""
    logger.debug('account suspended notification: %s', email_msg)

    account_id = email_msg.get('accountId')
    admin_emails = _get_admin_emails(email_msg.get(account_id))

    # fill in template
    filled_template = generate_template(current_app.config.get("TEMPLATE_PATH"), "account_suspended_email")
