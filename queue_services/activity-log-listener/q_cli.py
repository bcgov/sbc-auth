...
async def run(loop, payload: dict, is_replace: bool = False):  # pylint: disable=too-many-locals
    ...
    def subscription_options():
        return {
            'subject': os.getenv('NATS_SUBJECT', 'account.activity.events'),
            'queue': os.getenv('NATS_QUEUE', 'account.activity.events.worker'),
            'durable_name': os.getenv('NATS_QUEUE', 'account-activity-log-worker') + '_durable'
        }
