    ...
    if (payload.business.corpType.code === CorpTypes.INCORPORATION_APPLICATION) {
      ...
      if (filingResponse && filingResponse.data && filingResponse.status === 200) {
        ...
        if (filingId) {
          ...
          await OrgService.removeAffiliation(payload.orgIdentifier,
            payload.business.businessIdentifier || payload.business.nameRequest.nrNumber,
            payload.passcodeResetEmail, payload.resetPasscode)
        }
      }
    } else {
