<template>
  <div class="paybcreturn">
    <iframe :src="VUE_APP_COPS_REDIRECT_URL" ref="iframeContent" style="display: none"></iframe>
    <v-form ref="form" lazy-validation>
      <v-expand-transition>
        <div class="paybcreturn__alert-container" v-show="redirectError">
          <v-alert :value="true" color="error" icon="warning">{{redirectError}}</v-alert>
        </div>
      </v-expand-transition>
      <div class="paybcreturn__row"></div>
      <div class="paybcreturn__row"></div>
      <div class="paybcreturn__row paybcreturn__form-btns">
        <v-btn class="pay-btn" @click="returnTo" color="primary" large>
          <v-progress-circular :indeterminate="true" size="20" width="2" v-if="showSpinner"></v-progress-circular>
          <span>{{showSpinner ? 'Return to BC Regitries...' : 'Return to BC Regitries'}}</span>
...
export default {
  ...
  methods: {
    returnTo () {
      PaybcServices.get_payment_status('12345678')
        .then(response => {
