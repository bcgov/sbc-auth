      ...
      <template>
        <div class="mt-n2 mb-8">
          <v-btn
            ...
            class="mt-n6 px-0"
            @click="helpTextDashboard = !helpTextDashboard"
          >
            <v-icon>mdi-help-circle-outline</v-icon>
            <span>
              {{ helpTextDashboard ? 'Close Help' : 'Help with Starting a Business' }}
            </span>
          </v-btn>
        </div>

        <div v-if="helpTextDashboard" id="business-help-text" class="my-8 py-8">
...
export default class EntityManagement extends Mixins(AccountChangeMixin, NextPageMixin) {
  ...
  protected helpTextDashboard: boolean = false
  ...
  private get getAccountIdNameRequestUrl (): string {
    return appendAccountId(ConfigHelper.getNameRequestUrl())
  }

  private get getSelectorWizardUrl (): string {
    return ConfigHelper.getEntitySelectorUrl()
  }

  // open Name Request
  private goToNameRequest (): void {
    window.location.href = this.getAccountIdNameRequestUrl
  }
