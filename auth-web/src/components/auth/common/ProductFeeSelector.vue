...
export default class ProductFee extends Vue {
  ...
  @Watch('selectedApplyFilingFees', { deep: true })
    updateFilingFees (val, oldVal) {
      if (val?.existingApplyFilingFees.toString() !== oldVal?.existingApplyFilingFees.toString()) {
        this.applyFilingFees = val?.existingApplyFilingFees.toString() || ''
      }
      if (val?.existingserviceFeeCode.toString() !== oldVal?.existingserviceFeeCode.toString()) {
        this.serviceFeeCode = val?.existingserviceFeeCode || ''
      }
    }

  private mounted () {
    this.applyFilingFees = this.selectedApplyFilingFees?.existingApplyFilingFees.toString() || this.applyFilingFees
    this.serviceFeeCode = this.selectedApplyFilingFees?.existingserviceFeeCode || this.serviceFeeCode
  }
  ...
  @Emit('update:updatedProductFee')
    private selectChange () {
      const accountFee: AccountFee = {
        ...
        applyFilingFees: this.applyFilingFees,
